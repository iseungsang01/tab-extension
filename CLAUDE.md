...
- **`lib/tab_search.js`** — pure, dependency-free module exposing `normalizeTab`, `searchTabs`, `inferAutoTitle`, `isNonPaperTitleHost`, `classifyTab`, `cleanPaperTitle`, `extractArxivId`, `extractDoi`, `extractSilverchairDoi`, `extractPubmedId`, `tokenize`, `formatUrl`. Uses a UMD wrapper so it works both as a CommonJS `require` (tests) and as a `window.TabSearch` global (popup). **All logic that can be tested without a browser lives here**, and `test/tab_search.test.js` is the contract for it. When changing search ranking, title cleaning, or tab classification, update/extend these tests.
...
- **`autoTitle`** — derived by `inferAutoTitle`, which scores candidate titles from page metadata signals (`citationTitle`, `dcTitle`, `schemaTitle`, `ogTitle`, headings, filename) and strips site-name suffixes / arXiv-id noise via `cleanPaperTitle` + the `SITE_TITLE_PATTERNS` / `GENERIC_TITLES` heuristics. The goal is turning "Semantic Scholar - RAG Systems" into "RAG Systems". The highest-priority candidate is `paperTitle` — an authoritative title fetched out of band (see arXiv lookup below) — so any source that can resolve a real title just sets `paperTitle` and lets the existing ranking take over. Hosts in `NON_PAPER_TITLE_HOSTS` (YouTube and other media/social SPAs, matched by `isNonPaperTitleHost`) are exempted up front: `inferAutoTitle` returns `''` for them so the always-live tab title shows. This is deliberate — these sites are never papers and they leave social meta (`og:title` / `twitter:title`) stale after in-app navigation, so an inferred title would otherwise freeze on, say, the channel name you visited before opening a video. `popup.js`'s `canReadPageTitleSignals` calls the same predicate to skip injecting the metadata reader into those tabs at all.
- **`customTitle`** — a user-typed override, persisted per tab.

**External title lookup (popup layer).** The built-in PDF viewer and discarded tabs can't be read with `chrome.scripting.executeScript`, so `arxiv.org/pdf/<id>`, `doi.org/<doi>`, and `pubmed.ncbi.nlm.nih.gov/<pmid>` tabs would otherwise fall back to a bare id or filename. When `inferAutoTitle` yields nothing for such a tab, `applyAutoTitles` calls `resolveExternalTitle`, which walks the `TITLE_RESOLVERS` registry — each entry pairs a pure URL→id extractor (`extractArxivId` / `extractDoi` / `extractSilverchairDoi` / `extractPubmedId`) with a popup-layer fetcher (arXiv Atom API / Crossref `api.crossref.org/works` / NCBI `esummary`). The first source whose id appears in the URL wins; its title feeds back in as `paperTitle`. To add a source, add an extractor (with tests) and append one registry entry — nothing else changes. **`extractSilverchairDoi`** is a special case of the DOI path: Silverchair watermark PDFs (`watermark*.silverchair.com/<abbrev>.<year>.<seq>.pdf?token=…`) are the built-in PDF viewer *and* name the file after the DOI **suffix only**, so `extractDoi` (which needs a literal `10.x/…`) can't see it. `extractSilverchairDoi` rebuilds the full DOI from the leading journal abbreviation via the `SILVERCHAIR_DOI_PREFIXES` map (Royal Society family → `10.1098`; add a line per publisher) and reuses the Crossref fetcher under `source: 'doi'`, so its cache entry is shared with regular DOI tabs for the same paper. Results are cached by `"<source>:<id>"` in `chrome.storage.local` (`tabFinder.resolvedTitles.v1`, capped at `RESOLVED_TITLE_CACHE_LIMIT`); unlike tab-id-keyed data this cache survives close/reopen. Pages whose metadata already resolves a title (e.g. arXiv `/abs/` pages) skip the network call.
...
### Keyword memo prefill

On refresh, `fillKeywordMemos` seeds a paper tab's memo with the keywords printed on the paper. Sources are the **academic** keyword meta tags read by `collectPageTitleSignals` (`citation_keywords`, Dublin Core `dc.subject`/`dcterms.subject`, and JSON-LD `keywords` on scholarly types) — generic SEO `<meta name="keywords">` is deliberately excluded so non-papers don't get noisy memos. `applyAutoTitles` runs the raw signals through the pure `formatKeywords` (split on `,`/`;`/`|`/`•`, trim, dedupe case-insensitively, cap at `KEYWORD_MAX_COUNT`) and attaches the result as `autoKeywords`. These are not available on the built-in PDF viewer (no DOM) and Crossref/PubMed don't return keywords, so PDF-only tabs generally won't get them. The prefill is a **one-time, non-destructive offer per tab**: it writes only when the memo is empty, then records the tab in `tabFinder.keywordsFilled.v1` so editing or clearing the memo is never re-applied on the next refresh. Filled keywords are ordinary notes — searchable and counted as memos — per the design choice to write them in rather than keep a separate auto layer.

### Persistence

All user state is in `chrome.storage.local` under versioned keys defined at the top of `popup.js` (`tabFinder.tabNotes.v1`, `tabFinder.tabTitles.v1`, `tabFinder.favorites.v1`, `tabFinder.keywordsFilled.v1`, plus scope/language prefs). Keyed by stringified Chrome tab id. Notes/titles/favorites/keywords-filled are pruned on refresh for tabs that are no longer open (`pruneTabDataForTabs`). The one exception is the resolved-title cache (`tabFinder.resolvedTitles.v1`), which is keyed by `"<source>:<id>"` rather than tab id, is not pruned, and is bounded only by a size cap. Saves are debounced (`SAVE_DELAY_MS`) and flushed on `beforeunload` and before navigating to a tab. Because keys are tab ids, stored data does not survive tab close/reopen — this is intentional, not a bug.
