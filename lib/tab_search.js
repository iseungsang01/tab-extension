(function (root, factory) {
  ...
  // Media/social destinations that are never papers and behave as SPAs: they
  // navigate internally without a full reload, leaving social meta tags
  // (og:title / twitter:title) stale. E.g. after you open a video from a
  // channel page, YouTube keeps the channel's og:title, so an inferred title
  // would stick to the previous channel name. For these hosts we skip auto
  // titling entirely and let the always-live tab title show instead.
  const NON_PAPER_TITLE_HOSTS = [
    'youtube.com',
    'youtu.be',
    'twitch.tv',
    'netflix.com',
  ...
  function extractArxivId(url) {
    ...
    // Modern identifiers: 2301.01234 / 1706.03762v2 (optionally .pdf-suffixed).
    let match = path.match(/\/(?:abs|pdf|html|format)\/(\d{4}\.\d{4,5})(v\d+)?/i);
    if (match) return match[1] + (match[2] || '');

    // Legacy identifiers: hep-th/9901001, math.GT/0309136.
    match = path.match(/\/(?:abs|pdf|html|format)\/([a-z-]+(?:\.[a-z]{2})?\/\d{7})(v\d+)?/i);
    if (match) return match[1] + (match[2] || '');
  ...
  function extractDoi(url) {
    ...
    try {
      const parsed = new URL(raw);
      haystack = safeDecode(`${parsed.pathname || ''}${parsed.search || ''}`);
    } catch (_) {
      return '';
    }

    // Core DOI shape: 10.<registrant>/<suffix>. The suffix may contain slashes,
    // so trim trailing path noise (e.g. /fulltext, trailing slash, .pdf).
    const match = haystack.match(/10\.\d{4,9}\/[-._;()/:a-z0-9]+/i);
    if (!match) return '';
    return match[0]
      .replace(/\.pdf$/i, '')
      .replace(/\/(?:full|fulltext|abstract|pdf|epdf|references|citations)$/i, '')
      .replace(/[/.,;:]+$/, '');
  }

  // Silverchair watermark PDFs (e.g. watermark02.silverchair.com/rsta.2003.1227.pdf)
  // name the file after the DOI *suffix* only, dropping the registrant prefix.
  // We reconstruct the full DOI from the leading journal abbreviation. Every
  // Royal Society journal shares the 10.1098 prefix; add a line per publisher
  // to extend coverage.
  const SILVERCHAIR_DOI_PREFIXES = {
    rsta: '10.1098',
    rstb: '10.1098',
    rspa: '10.1098',
    rspb: '10.1098',
  ...
  function extractSilverchairDoi(url) {
    const raw = toText(url);
    if (!raw) return '';
    ...
    if (host !== 'silverchair.com' && !host.endsWith('.silverchair.com')) return '';

    const filename = (path.split('/').filter(Boolean).pop() || '').replace(/\.pdf$/i, '');
    // DOI suffix shape used by these journals: <abbrev>.<year>.<sequence>.
    const match = filename.match(/^([a-z]+)\.(\d{4}\.\d{3,6})$/i);
    if (!match) return '';

    const abbrev = lower(match[1]);
    const prefix = SILVERCHAIR_DOI_PREFIXES[abbrev];
    if (!prefix) return '';

    return `${prefix}/${abbrev}.${match[2]}`;
  }
  ...
  function extractPubmedId(url) {
    ...
    if (host === 'pubmed.ncbi.nlm.nih.gov') {
      const match = path.match(/^\/(\d{4,9})(?:\/|$)/);
      return match ? match[1] : '';
    }
    if (host === 'ncbi.nlm.nih.gov' || host.endsWith('.ncbi.nlm.nih.gov')) {
      const match = path.match(/\/pubmed\/(\d{4,9})(?:\/|$)/i);
      return match ? match[1] : '';
    }
    return '';
  }
  ...
  // Turns raw keyword signals (a comma/semicolon-packed string, or an array of
  // per-keyword <meta> contents) into a clean, deduped, comma-joined list for
  // the default memo. Returns '' when there is nothing usable.
  function formatKeywords(value) {
    ...
    for (const entry of entries) {
      const text = decodeBasicHtmlEntities(entry);
      if (!text) continue;

      // One tag may pack several keywords (",", ";", "|", "•"); multiple tags
      // arrive as separate array entries.
      for (const piece of text.split(/\s*[;,|•]\s*/)) {
        const keyword = normalizeSpaces(piece)
          .replace(/^[\s"'“”‘’`([{<]+/, '')
          .replace(/[\s"'“”‘’`)\]}>.]+$/, '')
          .trim();
        if (!keyword || keyword.length > KEYWORD_MAX_LENGTH) continue;

        const key = lower(keyword);
        if (seen.has(key)) continue;
        seen.add(key);
        keywords.push(keyword);
        if (keywords.length >= KEYWORD_MAX_COUNT) return keywords.join(', ');
      }
    }
  ...
  function isNonPaperTitleHost(host) {
    const normalized = lower(host);
    if (!normalized) return false;
    return NON_PAPER_TITLE_HOSTS.some(
  ...
  function inferAutoTitle(source) {
    const tab = source || {};
    if (isNonPaperTitleHost(parseUrl(tab.url).host)) return '';
